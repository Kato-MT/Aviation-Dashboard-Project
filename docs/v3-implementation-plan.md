...
The intended release description is a near-real-time regional airspace observation and investigation workbench. The checked-in pilot never starts a shared provider attempt more often than every 20 seconds. That cadence is not a promise that the underlying aircraft observations are 20 seconds old or that every aircraft is visible.
...
The main user journey should be explainable in 90 seconds: open the fixed Atlanta pilot, select a recently observed aircraft, inspect its received position and available altitude/speed history, then inspect source age and limitations. A separately selected synthetic replay demonstrates outage recovery reliably; the Lab demonstrates reproducible diagnostic reasoning. No account or live-provider availability should be required to demonstrate the synthetic workflows.
...
- Real-source pilot: Atlanta only, using `GET /v2/point/33.6407/-84.4277/100`, a no-faster-than-20-second shared attempt cadence, and at most 25 attached or closing WebSocket viewers. Savannah / Statesboro and Central Georgia remain synthetic-assurance presets only.
...
- Cloudflare Worker + one SQLite-backed Durable Object per active source region, one Atlanta object for the real-source pilot; R2 Standard for map assets.
...
At that checkpoint, `pnpm live:capacity` was an executable early estimate with 14 passing tests. Its then-current continuous three-region successful-poll baseline was 259,200 row writes/day, above the published 100,000 Free allowance before additional callers, cleanup and other workloads. The model labeled its source counts, one-continuous-window assumption and excluded costs. This remains historical evidence; the current Atlanta pilot model appears in the cost section below.
...
At that checkpoint, attempt reservation committed the polling alarm before upstream I/O. Socket admission also committed the newly required wakeup before returning the upgrade, including when it joined a REST-first request already in flight. This closed loss of the former ten-second polling obligation before final publication scheduling. Constructor recovery preserved existing alarms/retries and derived missing maintenance from stored keys. The then-current rollout plan required activation of all three fixed presets to wake legacy dormant objects. That is historical implementation evidence, not the current Atlanta-only real-source rollout scope.
...
At that checkpoint this was local acceptance of the delivery core, not all of M3.1. The following checkpoint added numeric route/control/map admission, guarded local native-fetch paths and the required Live CI definition. Process-wide egress claims, hosted CI evidence, physical memory/load behavior, actual platform metering, provider terms and the owner-approved operating envelope remained open. Its three-region, 100-viewer scenario is historical load evidence, not the current pilot envelope.

### M3.1 request-admission and local-CI checkpoint, 2026-08-28

At that checkpoint, fixed numeric policy bounded total API/map attempts, preflight, catalog, amplified three-region health calls, snapshots, stream handshakes, ranged map operations/bytes and open map-response streams before expensive downstream work. The counters used fixed route/region keys, contained no aircraft records or persistent client identity, and reset with the Worker isolate. Health concurrency remained held until all fanout branches settled, including partial failure. Snapshot bytes were charged against a maximum-response budget, and large map objects required an admitted bounded range. The three-region health case remains synthetic regression evidence; real-source health now exposes Atlanta only.
...
The regional coordinator is a small server-side object shared by everyone viewing the active region. It prevents each browser from independently polling the provider. The real-source pilot uses one deployment and one Atlanta coordinator, not one backend per person. Synthetic assurance may still instantiate all three retained Georgia presets without expanding the public provider surface.
...
| Route                                   | Behavior                                                                                                                   |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `GET /api/v1/regions`                   | Active source's fixed region catalog and declared capabilities; real source exposes Atlanta only; no aircraft data.        |
...
The nominal minimum interval between attempt starts is 20 seconds. Failure delays start at 20/40/60 seconds with positive jitter. Three consecutive failures open a nominal 60-second circuit. A longer provider Retry-After always wins, including HTTP-date headers. Do not cap it down to 60 seconds.

The next eligible attempt is the maximum of: the previous attempt-start plus 20 seconds; the provider's retry deadline; failure-completion time plus backoff and positive jitter; and the circuit deadline. Numeric Retry-After starts at response-header receipt, while HTTP-date Retry-After is an absolute deadline. Persist the resulting control state.
...
The hub now admits at most 25 attached or closing WebSocket viewers, uses one explicit application acknowledgment window per viewer and charges complete envelopes against an eight-MiB regional logical ledger. The installed Workers interface and documented `send()` API do not supply an application drain callback or declared `bufferedAmount` contract, so the implementation never treats a successful send as proof of receipt. [Workers WebSocket API](https://developers.cloudflare.com/workers/runtime-apis/websockets/#send)

1. At most 25 attached or closing sockets are admitted for the Atlanta pilot. Viewer 26 receives a bounded 503 response. Closing sockets remain charged conservatively until detached but receive no new data. This is a WebSocket-stream capacity rule, not a snapshot-reader cap, authentication, or an account-wide spending cap.
...
8. Local evidence covers 25 accepted/26 rejected, 24 progressing with one stalled viewer, all-viewers-stalled bounds, valid near-limit mixed-size eventual fairness, byte exhaustion/recovery, bounded coalescing, control floods, reconnect recovery, long-backoff ACK expiry, hibernation, the publication restart matrix and safe sequence exhaustion. None of these tests proves Cloudflare's internal transport buffers or physical memory; platform fan-out/memory proof remains G1.

Cloudflare documents a 128 MB memory limit per isolate, shared across concurrent work, not a separate allocation for every socket. Logical outstanding bytes, retained JavaScript objects, temporary serialization copies and internal transport memory are different quantities. Test one active Atlanta hub with 1, 10, and 25 WebSocket viewers, retain headroom and report what was actually measurable. A separate three-hub synthetic regression does not expand the public pilot envelope. Do not assert that the proposed 8 MiB regional credit alone proves memory safety. [Workers memory limit](https://developers.cloudflare.com/workers/platform/limits/#memory)
...
Use the existing reproducible PMTiles extract for the Atlanta public pilot. The retained archive may continue covering the union of the three checked-in Georgia bounds so synthetic assurance remains deterministic, but Savannah / Statesboro and Central Georgia are not real-source routes. Keep the zoom-12 cap and record source date, bounds, generator version, archive size, SHA-256 and licenses. Protomaps documents bounded extraction and archive verification. [PMTiles CLI](https://docs.protomaps.com/pmtiles/cli)
...
The health route asks every region exposed by the active source for status on every admitted request. Real-source mode exposes only Atlanta; synthetic assurance can expose all three retained presets. Snapshot and upgrade requests also reach the selected regional object. A shared provider poll does not make those HTTP invocations, control handlers or map reads free. Fixed request controls therefore run before expensive downstream work, not only around provider fetches.
...
- Health floods: measure and bound the one-object Atlanta public path; retain the three-object case as synthetic regression only. Rejected requests must not invoke downstream regional health calls.
- Upgrade/reconnect bursts: test the 25-WebSocket cap, closing sockets, refusal/retry behavior and sustained churn, not just 25 stable connections.
...
ADSB.lol remains the initial technical candidate. Its current public contract is a REST polling API, not a documented WebSocket or SSE feed. The checked-in pilot permits only `GET /v2/point/33.6407/-84.4277/100` and sends `GeorgiaFlightOutlook/0.1 (+https://github.com/Kato-MT/Aviation-Dashboard-Project)`. The current API application source declares the API and public data under ODbL 1.0. Treat the license label as established by that source while keeping the exact attribution, share-alike, browser-redistribution, Produced Work and derived-database compliance steps open for provider coordination and release review. [Current API repository](https://github.com/adsblol/api/), [current API license declaration](https://github.com/adsblol/api/blob/main/src/adsb_api/app.py), [privacy and license page](https://www.adsb.lol/privacy-license/)

Its repository describes dynamic rate limits, not a guaranteed numerical quota. ADSB.lol supplied general operating guidance on 2026-08-30: visible ODbL attribution, a contact-bearing User-Agent, respectful HTTP-error and `Retry-After` handling, shared caching and deduplication, no rate-limit circumvention, and no availability guarantee. The response did not approve the proposed 20-second cadence, 25-viewer ceiling, exact User-Agent, public no-key access, incident contact, change-notification channel, or transient normalized redistribution obligations. [Provider rate-limit guidance](https://github.com/adsblol/api)

The 2026-08-30 response closes the initial-contact substep only. Before production, resolve the remaining exact-envelope questions, recheck current terms, fields, limits, API-key policy and visible attribution, and record the owner-approved outcome. The written result must cover the Atlanta endpoint, no-faster-than-20-second cadence, 25-WebSocket-viewer ceiling, Cloudflare egress, browser redistribution, attribution, retention and derived-database obligations. It is not permission to enable Live or make a real-aircraft request. Any follow-up message or acceptance of terms still requires separate owner approval.
...
At one shared attempt every 20 seconds, the continuously viewed Atlanta pilot makes at most 4,320 scheduled provider attempts per day. Viewer count must not multiply that number. This is a calculated maximum, not measured traffic or a provider-approved quota.
...
The executable capacity check now includes bounded delivery, ACK/ping/pong traffic and explicit reconnect attempts, accepts and rejects. The published 100,000-row-write allowance divided by 4,320 daily Atlanta attempts gives about 23.15 billed row writes per attempt before maintenance, indexes or other workloads. This calculated ceiling is not a per-poll allocation or permission to consume the whole allowance. Count write amplification and measure active duration, then seek a small controlled platform measurement when authorized account access permits. Actual billed measurements remain mandatory before live enablement.

Make that estimate executable. Count alarm writes and hidden key-value storage writes, not just application SQL statements. A hypothetical four-write successful poll would consume `4,320 * 4 = 17,280` writes/day. This is a design-risk calculation, not a measurement of the current code. Consolidate state where correctness allows; never remove durable ordering or expiry controls to meet a quota. [Storage metering rules](https://developers.cloudflare.com/durable-objects/platform/pricing/)

The shared-cadence implementation currently has a source-counted success baseline of nine KV-row writes plus two alarm-row writes per scheduled attempt: one alarm arms the 10-second ACK deadline and the later ACK path restores the 20-second provider deadline. The local method-count verification therefore models 11 writes per success. At continuous 20-second cadence, Atlanta implies at least `4,320 * 11 = 47,520` row writes/day before initialization, early alarms, cleanup, failures, snapshot callers and other workloads. This is below the published Free allowance but is not platform billing evidence or proof of adequate headroom. Obtain physical measurement and an explicit operating-envelope/budget decision before enabling Live.

The capacity model includes coalescing inputs, ACKs, keepalives, pongs, connection attempts, accepted/rejected reconnects, ACK-alarm changes and source-counted storage work. One ACK for each 20-second snapshot would add at most 108,000 incoming application messages for 25 continuously connected Atlanta WebSocket viewers per day, before keepalives and separately acknowledged control deliveries. These remain calculated traffic scenarios, not measured throughput, active duration, physical memory or billing.

The checked-in peak is 25 attached or closing WebSocket viewers for the Atlanta pilot. Snapshot HTTP readers remain subject to separate API admission controls and do not consume this stream slot count. Model region-active hours, viewer connection-hours, incoming keepalives, poll latency, serialization/fan-out time, map requests, and daily storage operations. The release records the measured operating envelope and owner-approved budget. If they conflict with the target, stop public enablement for an explicit decision while local mock/replay/Lab work continues.

Before public enablement:

- Measure one Atlanta hub with 1, 10, and 25 WebSocket viewers, reconnect bursts, tile cold starts, normal provider latency and timeout/backoff periods. Any retained three-region synthetic regression is test coverage, not public scope.
...
M3.1 preserves the completed executable model and prepares an explicitly constrained test envelope; G1 then supplies physical measurements for Kato's production decision. Any later write consolidation must preserve every durability and retention guarantee. A proposed constraint may further limit active usage, but must not start shared provider attempts more often than every 20 seconds, admit more than 25 attached or closing WebSocket viewers, expand the real-source surface beyond Atlanta, relabel stale evidence, or remove the retained presets from synthetic assurance.
...
The 25-WebSocket-viewer ceiling is the pilot's tested peak-capacity target, not an entitlement to continuous free operation. Full local product work can continue while the owner decides the external operating budget.
...
| G2: bounded real-provider smoke    | Remaining provider questions and source/license review complete; Kato approves the exact environment, Atlanta endpoint, no-faster-than-20-second cadence, 25-WebSocket-viewer ceiling, request ceiling, test duration and aggregate-only retention policy. Any email requires approval of recipient and wording separately. | Nonempty source-age, status, count and latency evidence from the real deployed pipeline, with raw payload/trace/screenshot retention disabled. An empty valid feed is connectivity evidence only and is distinguished from unavailable. | Mock outage/recovery/load tests, accessibility, offline verification and artifact preparation. Never substitute another live provider without review. |
...
- The proposed operating envelope: Atlanta only; `GET /v2/point/33.6407/-84.4277/100`; at least 20 seconds between shared attempt starts; at most 25 attached or closing WebSocket viewers; expected daily active/viewer hours; request and storage estimates; budget-warning points; and the procedure for disabling Live. Any paid plan or material capacity tradeoff requires Kato's decision.
...
- After the remaining provider questions close, a proposed real smoke of Atlanta only, at most five minutes and 15 upstream attempts, with an immediate stop on a rate-limit response or unexpected access restriction. These are suggested approval limits, not permission to run the test. Capture only aggregate results; fault/retry stress belongs to the mock test.
...
1. Open the fixed Atlanta pilot and select an observation from the real map or accessible aircraft table.
...
| Time                 | Exact 15/45/120-second boundaries; a 60-second cached snapshot plus 20-second delivery with a wrong browser clock; server-offset uncertainty/expiry; provider regression; future/missing time; removal without new data.                       |
...
| Regional coordinator | 25 staggered WebSocket clients, mixed HTTP/WS requests, one eligible poll, duplicate alarms, zero viewers, cold cache, hibernation/eviction, circuit recovery, and crashes around reservation, durable publication commit and partial fan-out. |
...
- Exercise worst-case payload fan-out to 25 synthetic WebSocket viewers, including one stalled client. Application ACK bounds and actual browser/platform memory are separate measurements.
- Exercise all viewers stalled and mixed payload sizes while enforcing the configured regional logical-byte cap. Healthy ready viewers must receive service without starvation after credit returns. Measure one Atlanta hub, transient serialization allocations and runtime errors separately from logical-byte counters; retained three-hub synthetic regression does not expand the public pilot. Do not infer heap safety from a per-socket frame count.
...
7. **Request G1 and G2 separately:** G1 requires Kato's Cloudflare account, budget, R2 and Durable Object activation, and bounded metered-run approval. ADSB.lol's 2026-08-30 response closed only the initial-contact substep. G2 still requires resolution of the exact Atlanta endpoint, 20-second cadence, 25-WebSocket-viewer ceiling, privacy, attribution, no-key access, incident contact and redistribution questions, plus exact test-envelope approval and aggregate-only evidence. Do not send a follow-up provider message or accept terms without separate approval.
