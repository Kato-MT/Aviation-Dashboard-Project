...
Status: authoritative roadmap, local R3 implementation exit passes for its recorded dirty source, R4 acceptance and external release remain open

Product direction: A+B, regional Live Airspace with an evidence-first light workspace and selected-track investigation

Repository checkpoint: `35cc3a981a59d3341b2a4b10dfe78730c9636e8e` on `feat/live-airspace-v3`, dirty working tree
...
The architecture should not be reset. The current provider-neutral React, Worker, Durable Object, MapLibre, PMTiles, Replay, Lab, Evidence, and unified offline foundation is the correct shape. Local R3 hardening is implemented, and the 2026-09-22 strict-v3 R3-05 browser-performance run passed all four expected cases for its recorded dirty-source identity. Frozen-source and retained-byte acceptance remain downstream, followed by separately authorized platform, real-source, and release gates.
...
| Browser evidence    | The complete dirty-tree browser matrix, deterministic desktop and mobile A+B visual baselines, browser-persistence inspection, bundle budgets, 500-aircraft paint measurements, and 2,000-record browser-safety gates pass locally. The strict-v3 aggregate passed 4/4 cases with zero retries or policy, identity, and environment violations; desktop/mobile paint p95 measured 116.5/158.1 milliseconds, and all ten interaction p95 values remained below 584 milliseconds. React Investigation, Campaign, and unified offline are linked under `TC-LUI-016..018`. | The same browser, visual, persistence, performance, fresh load, and fresh soak proof must repeat against one frozen retained candidate.                                       |
...
ADSB.lol is the selected primary provider basis for the bounded pilot under its published API guidance and authenticated provider reply. It is not a guaranteed or safety-critical dependency.

ADSB.lol coordination response received on 2026-08-30. The provider confirmed visible ODbL attribution, an identifiable contact-bearing User-Agent, respectful handling of HTTP errors and `Retry-After`, shared caching and deduplication, no rate-limit circumvention, and no service-level or stability guarantee. Together with the provider's published guidance, that authenticated reply is the operating basis for the bounded Atlanta pilot. No second bespoke acceptance email is required or planned. G2 remains pending because no real-source trial has run, and Live remains disabled. The sanitized repository record is [`docs/provider/adsb-lol-coordination-2026-08-30.json`](provider/adsb-lol-coordination-2026-08-30.json).

Before public use, validate and record adherence to:
...
| R3 browser hardening       | Visual baselines and bundle budgets have local proof. The 2026-09-22 strict-v3 run passed all four desktop/mobile paint and 2,000-record interaction cases with an eligible environment, zero retries, and zero policy, identity, or environment violations. |
...
- At 500 visible aircraft, disable MapLibre symbol fading, complete five warmups, then measure thirty validated-snapshot-to-stable-render barriers in three fixed blocks of ten. Nearest-rank p95 remains at or below 500 ms on the recorded desktop profile and at or below 750 ms on the recorded mobile profile, with no more than one over-budget sample. Preserve every raw barrier, DOM, map, validation, and wire-size sample plus four diagnostic scheduling-control blocks. The control has no baseline yet, so it may classify a failure but must never normalize, excuse, or convert the product result. The barrier covers React stabilization and MapLibre idle plus a subsequent animation-frame callback; it does not claim direct observation of physical display presentation.
- At 2,000 accepted records, no crash, truncation, unbounded queue, or loss of filter, focus, selection, and table access. Search, select, sort, close-focus restoration, and scroll each run from an equivalent restored state with two warmups and twenty measured keyboard interactions; every nearest-rank p95 remains at or below 1,000 ms with no more than one over-budget sample. Record privacy-minimized browser, WebGL renderer-class, CPU-class, logical-concurrency, memory-class, and runner-image classifications without detailed hardware fingerprints or any weakening of product budgets.
...
The published ADSB.lol guidance and authenticated 2026-08-30 reply close the provider-basis substep. G2 still requires Kato's authorization for the exact bounded real-source execution, but it does not wait for a second custom provider approval.

Preconditions requiring current documentation and Kato approval:

- dated published provider guidance plus the authenticated coordination record;
- exact `GET /v2/point/33.6407/-84.4277/100` endpoint, no-faster-than-20-second shared cadence, 25-WebSocket-viewer ceiling, attribution, cache, redistribution, retention, and current authentication decisions;
...
- No cadence, region, provider, or duration may expand without a new documented guidance review and Kato's approval.
...
9. Completed locally for the recorded dirty development source: the selected [hardening pair](../hardening/r3/hardening.md), D1, and R3-01 through R3-06 are implemented. Stable requirements, source-bound evidence, artifact privacy, aggregate-only G2 firebreak, typed numeric policy, deterministic visual baselines, eight executable runbooks, candidate-bound receipt verification, and the passing four-case strict-v3 R3-05 aggregate are in place. This closes the local R3 implementation exit without claiming frozen-source, retained-candidate, hosted, provider, deployment, or release proof.
