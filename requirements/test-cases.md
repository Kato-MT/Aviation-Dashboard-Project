...
| ID         | Behavior and expected result                                                                                                                                                                                   | Source linkage                                                          |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| TC-LIV-001 | The live region catalog exposes exactly the three documented Georgia presets and rejects unknown region identifiers.                                                                                           | Linked: `tests/live`, `tests/worker/worker.test.ts`                     |
...
| TC-LIV-006 | Regional session state ignores duplicate or out-of-order snapshots, bounds points and aircraft trails, emits quality transitions once, preserves explicit freshness states, and rejects cross-region evidence. | Linked: `tests/live/session.test.ts`                                    |
| TC-LIV-007 | Presentation and runtime state deterministically filter, sort, summarize, project, format, select, reconnect, age, and reset regional aircraft evidence without fabricating missing measurements.              | Linked: `tests/live/presentation.test.ts`, `tests/live/runtime.test.ts` |
...
| TC-EDG-003 | Rate limits and provider failures persist retry or circuit state and prevent repeated snapshot requests from hammering the upstream provider.                                                                 | Linked: `tests/worker/worker.test.ts` |
...
| TC-EDG-006 | A Worker dry-run bundles static assets, the regional Durable Object, fixed environment variables, and the versioned API without an undeclared binding.                                                        | Linked: `wrangler deploy --dry-run`   |

### v3.0 live browser release cases

| ID         | Behavior and expected result                                                                                                                                                                                                  | Source linkage                                            |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| TC-LUI-001 | Live Airspace is the default view and the existing Diagnostics Lab, Investigation, Verification, Configuration, and evidence capabilities remain reachable and functional.                                                    | Pending: `tests/browser/live-airspace.spec.ts`            |
| TC-LUI-002 | Map, region, search, filter, sort, aircraft list, selected evidence, bounded trails, and selected session charts update from validated runtime state and handle an empty region.                                              | Pending: `tests/browser/live-airspace.spec.ts`            |
| TC-LUI-003 | Loading, live, degraded, stale, reconnecting, offline, rejected-message, and upstream-error presentations retain or clear evidence according to the declared state rules and never claim aircraft health or safety diagnoses. | Pending: `tests/browser/live-airspace-failures.spec.ts`   |
| TC-LUI-004 | Live controls and evidence are keyboard-operable, named, visibly focused, announced without color-only meaning, reduced-motion safe, and free of serious or critical automated accessibility findings.                        | Pending: `tests/accessibility/live-airspace.spec.ts`      |
| TC-LUI-005 | Live evidence remains usable at mobile, desktop, and 200 percent zoom layouts without hiding required status, selection, or feed-boundary copy.                                                                               | Pending: `tests/browser/live-airspace-responsive.spec.ts` |
| TC-LUI-006 | With network disabled, the offline build keeps the synthetic lab functional, labels Live Airspace unavailable, and makes no provider, map-tile, analytics, CDN, or telemetry-upload request.                                  | Pending: `tests/browser/offline-live-airspace.spec.ts`    |
